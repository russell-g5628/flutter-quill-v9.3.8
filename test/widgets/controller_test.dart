...
void main() {
  ...
  setUp(() {
    controller = QuillController.basic()
      ..compose(Delta()..insert(testDocumentContents), const TextSelection.collapsed(offset: 0),
          ChangeSource.local);
  });
  ...
  group('controller', () {
    ...
    test('getAllIndividualSelectionStylesAndEmbed', () {
      controller
        ...
        ..updateSelection(const TextSelection(baseOffset: 0, extentOffset: 4), ChangeSource.remote);
    ...
    test('getAllIndividualSelectionStylesAndEmbed mixed', () {
      ...
      expect(controller.getPlainText(), 'ld plain ita', reason: 'Selection spans 3 styles');
      //
      final result = controller.getAllIndividualSelectionStylesAndEmbed();
      expect(result.length, 2);
      expect(result[0].offset, 0);
    ...
    test('compose', () {
      ...
      controller
        ...
        ..compose(
            Delta()..insert('test '), const TextSelection.collapsed(offset: 0), ChangeSource.local);

      expect(listenerCalled, isTrue);
      expect(controller.document.toDelta(), Delta()..insert('test $originalContents'));
    });
